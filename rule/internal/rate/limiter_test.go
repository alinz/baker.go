...
func TestLimit(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := rate.LimitAll(tt.requestsLimit, tt.windowLength)(h)
...
func TestWithIncrement(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, code := range tt.respCodes {
				req := httptest.NewRequest("GET", "/", nil)
				req = req.WithContext(rate.WithIncrement(req.Context(), 2))
				recorder := httptest.NewRecorder()
				router.ServeHTTP(recorder, req)
				if respCode := recorder.Result().StatusCode; respCode != code {
					t.Errorf("resp.StatusCode(%v) = %v, want %v", i, respCode, code)
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router := rate.Limit(
				tt.requestsLimit,
				tt.windowLength,
				rate.WithLimitHandler(func(w http.ResponseWriter, r *http.Request) {
...
func TestLimitIP(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {})
			router := rate.LimitByIP(tt.requestsLimit, tt.windowLength)(h)
